...
class AcronymRenderScene:
    def __init__(self, global_config):
        ...
        self.scene_contacts_dir, self.valid_scene_contacts = self._check_scene_contacts(
            self.data_path, global_config["DATA"]["scene_contacts_path"]
        )
    ...
    def render_scene(self, scene_id):
        ...
        scene_info = np.load(
            os.path.join(self.scene_contacts_dir, scene_id + ".npz"), allow_pickle=False
        )
        ...
        self.change_scene(
            renderer, obj_paths, obj_scales, obj_transforms, visualize=False
        )
        # try:
        ...
        for i, cam_ori in enumerate(pbar):
            ...
            pc_cam, pc_normals, camera_pose, depth = self.render_pose(
                renderer, camera_pose, estimate_normals=self._estimate_normals
            )
    ...
    def change_scene(
        self, renderer, obj_paths, obj_scales, obj_transforms, visualize=False
    ):
        """
    ...
    def render_pose(self, renderer, camera_pose, estimate_normals=False):
        ...
        pc = self._regularize_pc_point_count(
    ...
    def _check_scene_contacts(
        self, dataset_folder, scene_contacts_path="scene_contacts_new"
    ):
        """
    ...
    def _regularize_pc_point_count(pc, npoints, use_farthest_point=False):
        ...
        if pc.shape[0] > npoints:
            if use_farthest_point:
                _, center_indexes = utils.farthest_points(
                    pc,
                    npoints,
...
def render_acronym(global_config):
    ...
    scene_contacts_dir, valid_scene_contacts = check_scene_contacts(
        data_path, global_config["DATA"]["scene_contacts_path"]
    )
