...
class GraspEstimator:
    ...
    def __init__(self, cfg):
        ...
        self._model_func = importlib.import_module(
            "contact_graspnet_pytorch." + self._contact_grasp_cfg["MODEL"]["model"]
        )
    ...
    def build_network(self):
        ...
        self.model_ops = {
            ...
            "offset_pred_idcs_pc": (
                tf.argmax(end_points["grasp_offset_head"], axis=2)
                if global_config["MODEL"]["bin_offsets"]
                else None
            ),
    ...
    def predict_grasps(
        self, pc, constant_offset=False, convert_cam_coords=True, forward_passes=1
    ):
        """
        ...
        if local_regions:
            ...
            if use_cam_boxes:
                pc_regions, _ = self.extract_3d_cam_boxes(
                    pc_full, pc_segments, min_size=0.2
                )
            else:
            ...
            pred_grasps_cam[-1], scores[-1], contact_pts[-1], gripper_openings[-1] = (
                self.predict_grasps(
                    pc_full, convert_cam_coords=True, forward_passes=forward_passes
                )
            )
