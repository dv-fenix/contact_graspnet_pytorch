...
def build_summary_ops(ops, sess, global_config):
    ...
    tf.summary.scalar(
        "offset_loss_x%s" % global_config["OPTIMIZER"]["offset_loss_weight"],
        ops["offset_loss"],
    )
    ...
    if global_config["MODEL"]["bin_offsets"]:
        ...
        tf.summary.scalar(
            "offset_bin_classification_accuracy",
            tf.reduce_mean(
                tf.cast(
                    tf.equal(ops["offset_pred_idcs_pc"], ops["offset_label_idcs_pc"]),
                    tf.float32,
                )
            ),
        )
        ...
        tf.summary.scalar(
            "offset_mean_abs_error",
            tf.reduce_mean(
                tf.abs(
                    bin_mean_preds
                    - tf.layers.flatten(tf.abs(ops["offset_orig_labels_vals"]))
                )
            ),
        )
        tf.summary.scalar(
            "offset_max_abs_error",
            tf.reduce_max(
                tf.abs(
                    bin_mean_preds
                    - tf.layers.flatten(tf.abs(ops["offset_orig_labels_vals"]))
    ...
    auc, auc_update_op = tf.metrics.auc(
        tf.layers.flatten(tf.cast(ops["grasp_suc_labels_pc"], tf.bool)),
        tf.layers.flatten(ops["binary_seg_pred"]),
        curve="PR",
        summation_method="careful_interpolation",
    )
    tf.summary.scalar("AUCPR", auc)

    pr_curve_streaming_func = (
        summary_lib.v1.pr_curve_streaming_op
        if TF2
        else summary_lib.pr_curve_streaming_op
    )
    _, pr_update_op = pr_curve_streaming_func(
        name="pr_contact_success",
    ...
    summary_ops = {
        ...
        "pr_update_op": pr_update_op,
        "auc_update_op": auc_update_op,
        "acc_update_ops": acc_update_ops,
        "pr_reset_op": pr_reset_op,
    }
    return summary_ops
