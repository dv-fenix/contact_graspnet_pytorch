...
def visualize_grasps(
    ...
    if plot_opencv_cam:
        plot_coordinates(
            ...
            np.eye(3, 3),
            central_color=(0.5, 0.5, 0.5),
        )
        # This is world in cam frame
        T_cam_world = np.linalg.inv(
            T_world_cam
        )  # We plot everything in the camera frame
        t = T_cam_world[:3, 3]
        r = T_cam_world[:3, :3]
    ...
    for i, k in enumerate(pred_grasps_cam):
        if np.any(pred_grasps_cam[k]):
            ...
            if len(pred_grasps_cam) > 1:
                ...
                colors3 = [
                    cm2((score - min_score) / (max_score - min_score))[:3]
                ...
                draw_grasps(
                    vis,
                    [pred_grasps_cam[k][best_grasp_idx]],
                    np.eye(4),
                    colors=[(1, 0, 0)],
                    gripper_openings=gripper_openings_k,
                )
