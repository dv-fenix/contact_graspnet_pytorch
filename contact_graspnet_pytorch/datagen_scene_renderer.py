...
class SceneRenderer:
    ...
    def __init__(
        self, intrinsics=None, fov=np.pi / 6, caching=True, viewing_mode=False
    ):
    ...
    def _init_camera_renderer(self):
        ...
        if self._intrinsics in ["kinect_azure", "realsense"]:
            ...
            self.renderer = self.pyrender.OffscreenRenderer(
                viewport_width=self._width, viewport_height=self._height, point_size=1.0
            )
        else:
            camera = self.pyrender.PerspectiveCamera(
    ...
    def render_labels(self, full_depth, obj_paths, obj_scales, render_pc=False):
        ...
        obj_names = [
            path + "_" + str(scale) for path, scale in zip(obj_paths, obj_scales)
        ]
