...
class ContactGraspnet(nn.Module):
    def __init__(self, global_config, device):
        ...
        mlp_list_0 = self.model_config["pointnet_sa_modules_msg"][0][
            "mlp_list"
        ]  # list of lists
        ...
        if (
            "asymmetric_model" in self.model_config
            and self.model_config["asymmetric_model"]
        ):
            ...
            self.fp3 = PointNetFeaturePropagation(
                in_channel=sa_mlp[-1] + sa3_out_channels, mlp=fp_mlp_2
            )

            self.fp2 = PointNetFeaturePropagation(
                in_channel=fp_mlp_2[-1] + sa2_out_channels, mlp=fp_mlp_1
            )

            self.fp1 = PointNetFeaturePropagation(
                in_channel=fp_mlp_1[-1] + sa1_out_channels, mlp=fp_mlp_0
            )
        else:
    ...
    def forward(self, point_cloud):
        ...
        if (
            "raw_num_points" in self.data_config
            and self.data_config["raw_num_points"]
            != self.data_config["ndataset_points"]
        ):
            raise NotImplementedError
        ...
        point_cloud = torch.transpose(
            point_cloud, 1, 2
        )  # Now we have batch x channels (3 or 6) x num_points
        ...
        grasp_dir_head_normed = F.normalize(
            grasp_dir_head, p=2, dim=1
        ...
        dot_product = torch.sum(
            grasp_dir_head_normed * approach_dir_head, dim=1, keepdim=True
        )
        projection = dot_product * grasp_dir_head_normed
        approach_dir_head_orthog = F.normalize(
            approach_dir_head - projection, p=2, dim=1
        )
        ...
        if self.model_config["bin_offsets"]:
            ...
            offset_bin_pred_vals = torch_bin_vals[
                argmax_indices
        ...
        # TODO Start here
        pred_grasps_cam = self.build_6d_grasp(
            approach_dir_head_orthog.permute(0, 2, 1),
    ...
    def get_bin_vals(self):
        ...
        if not self.global_config["TEST"]["allow_zero_margin"]:
            bin_vals = np.minimum(
                bin_vals,
                self.global_config["DATA"]["gripper_width"]
    ...
    def build_6d_grasp(
        ...
        if use_torch:
            grasp_R = torch.stack(
                [base_dirs, torch.cross(approach_dirs, base_dirs), approach_dirs], dim=3
            )  # B x N x 3 x 3
            grasp_t = (
                contact_pts
                + (thickness / 2) * base_dirs
                - gripper_depth * approach_dirs
            )  # B x N x 3
            grasp_t = grasp_t.unsqueeze(3)  # B x N x 3 x 1
            ones = torch.ones(
                (contact_pts.shape[0], contact_pts.shape[1], 1, 1), dtype=torch.float32
            ).to(
                self.device
            )  # B x N x 1 x 1
            zeros = torch.zeros(
                (contact_pts.shape[0], contact_pts.shape[1], 1, 3), dtype=torch.float32
            ).to(
                self.device
            )  # B x N x 1 x 3
            homog_vec = torch.cat([zeros, ones], dim=3)  # B x N x 1 x 4
            grasps = torch.cat(
                [torch.cat([grasp_R, grasp_t], dim=3), homog_vec], dim=2
            )  # B x N x 4 x 4
            ...
            for i in range(len(contact_pts)):
                ...
                grasp[:3, 3] = (
                    contact_pts[i]
                    + thickness[i] / 2 * grasp[:3, 0]
                    - gripper_depth * grasp[:3, 2]
                )
...
class ContactGraspnetLoss(nn.Module):
    def __init__(self, global_config, device):
        ...
        gripper_control_points = self.gripper.get_control_point_tensor(
            n_copies
        )  # b x 5 x 3
        sym_gripper_control_points = self.gripper.get_control_point_tensor(
            n_copies, symmetric=True
        )

        self.gripper_control_points_homog = torch.cat(
            [
                gripper_control_points,
                torch.ones((n_copies, gripper_control_points.shape[1], 1)),
            ],
            dim=2,
        )  # b x 5 x 4
        self.sym_gripper_control_points_homog = torch.cat(
            [
        ...
        self.gripper_control_points_homog = self.gripper_control_points_homog.to(
            self.device
        )
        self.sym_gripper_control_points_homog = (
            self.sym_gripper_control_points_homog.to(self.device)
        )
    ...
    def forward(self, pred, target):
        ...
        min_geom_loss_divisor = float(
            self.global_config["LOSS"]["min_geom_loss_divisor"]
        )  # This is 1.0
        pos_grasps_in_view = torch.clamp(
            grasp_success_labels_pc.sum(dim=1), min=min_geom_loss_divisor
        )  # B
        ...
        if self.score_ce_loss_weight > 0:  # TODO (bin_ce_loss)
            bin_ce_loss = F.binary_cross_entropy(
                pred_scores, grasp_success_labels_pc, reduction="none"
            )  # B x N x 1
            if (
                "topk_confidence" in self.global_config["LOSS"]
                and self.global_config["LOSS"]["topk_confidence"]
            ):
                bin_ce_loss, _ = torch.topk(
                    bin_ce_loss.squeeze(),
                    k=self.global_config["LOSS"]["topk_confidence"],
                )
        ...
        if self.offset_loss_weight > 0:  # TODO  (offset_loss)
            if self.global_config["MODEL"]["bin_offsets"]:
                ...
                grasp_offset_labels_multihot = self._bin_label_to_multihot(
                    grasp_offset_labels_pc, bin_vals
                )

                if (
                    self.global_config["LOSS"]["offset_loss_type"]
                    ...
                    offset_loss = F.binary_cross_entropy_with_logits(
                        grasp_offset_head,
                        grasp_offset_labels_multihot,
                        reduction="none",
                    )  # B x N x 1
                    if (
                        "too_small_offset_pred_bin_factor" in self.global_config["LOSS"]
                        and self.global_config["LOSS"][
            ...
            offset_loss = torch.mean(
                torch.sum(masked_offset_loss, axis=1, keepdim=True) / pos_grasps_in_view
            )
        ...
        if self.adds_loss_weight > 0:  # TODO  (adds_loss)
            ...
            if self.global_config["MODEL"]["bin_offsets"]:
                thickness_gt = self.bin_vals[
                    torch.argmax(grasp_offset_labels_pc, dim=2)
                ]
            else:
            ...
            gt_grasps_proj = utils.build_6d_grasp(
                approach_labels_pc_cam,
                dir_labels_pc_cam,
                pred_points,
                thickness_gt,
            ...
            success_mask = torch.broadcast_to(
                success_mask, gt_grasps_proj.shape
            )  # B x N x 4 x 4
            pos_gt_grasps_proj = torch.where(
                success_mask, gt_grasps_proj, torch.ones_like(gt_grasps_proj) * 100000
            )  # B x N x 4 x 4

            # Expand gripper control points to match number of points
            # only use per point pred grasps but not per point gt grasps
            control_points = self.gripper_control_points_homog.unsqueeze(
                1
            )  # 1 x 1 x 5 x 4
            control_points = control_points.repeat(
                pred_points.shape[0], pred_points.shape[1], 1, 1
            )  # b x N x 5 x 4

            sym_control_points = self.sym_gripper_control_points_homog.unsqueeze(
                1
            )  # 1 x 1 x 5 x 4
            sym_control_points = sym_control_points.repeat(
                pred_points.shape[0], pred_points.shape[1], 1, 1
            )  # b x N x 5 x 4

            pred_control_points = torch.matmul(
                control_points, pred_grasps.permute(0, 1, 3, 2)
            )[
                :, :, :, :3
            ]  # b x N x 5 x 3

            # Transform control points to ground truth locations
            gt_control_points = torch.matmul(
                control_points, pos_gt_grasps_proj.permute(0, 1, 3, 2)
            )[
                :, :, :, :3
            ]  # b x N x 5 x 3
            sym_gt_control_points = torch.matmul(
                sym_control_points, pos_gt_grasps_proj.permute(0, 1, 3, 2)
            )[
                :, :, :, :3
            ]  # b x N x 5 x 3

            # Compute distances between predicted and ground truth control points
            expanded_pred_control_points = pred_control_points.unsqueeze(
                2
            )  # B x N x 1 x 5 x 3
            expanded_gt_control_points = gt_control_points.unsqueeze(
                1
            )  # B x 1 x N' x 5 x 3  I think N' == N
            expanded_sym_gt_control_points = sym_gt_control_points.unsqueeze(
                1
            )  # B x 1 x N' x 5 x 3  I think N' == N

            # Sum of squared distances between all points
            squared_add = torch.sum(
                (expanded_pred_control_points - expanded_gt_control_points) ** 2,
                dim=(3, 4),
            )  # B x N x N'
            sym_squared_add = torch.sum(
                (expanded_pred_control_points - expanded_sym_gt_control_points) ** 2,
                dim=(3, 4),
            )  # B x N x N'

            # Combine distances between gt and symmetric gt grasps
            squared_adds = torch.concat(
                [squared_add, sym_squared_add], dim=2
            )  # B x N x 2N'

            # Take min distance to gt grasp for each predicted grasp
            squared_adds_k = torch.topk(squared_adds, k=1, dim=2, largest=False)[
                0
            ]  # B x N
            ...
            sum_grasp_success_labels = torch.sum(
                grasp_success_labels_pc, dim=2, keepdim=True
            )
            binary_grasp_success_labels = torch.clamp(sum_grasp_success_labels, 0, 1)
            min_adds = binary_grasp_success_labels * torch.sqrt(
                squared_adds_k
            )  # B x N x 1
            adds_loss = torch.sum(
                pred_scores * min_adds, dim=(1), keepdim=True
            )  # B x 1
    ...
    def _bin_label_to_multihot(self, cont_labels, bin_boundaries):
        ...
        for b in range(len(bin_boundaries) - 1):
            bins.append(
                torch.logical_and(
                    torch.greater_equal(cont_labels, bin_boundaries[b]),
                    torch.less(cont_labels, bin_boundaries[b + 1]),
                )
            )
    ...
    def _compute_labels(
        ...
        pos_contact_points_cam = (
            torch.matmul(pos_contact_points, camera_poses[:, :3, :3].transpose(1, 2))
        ...
        pos_contact_dirs_cam = torch.matmul(
            pos_contact_dirs, camera_poses[:, :3, :3].transpose(1, 2)
        )
        ...
        pos_approach_dirs_cam = torch.matmul(
            pos_approach_dirs, camera_poses[:, :3, :3].transpose(1, 2)
        )
        ...
        if filter_z:
            ...
            dir_filter_passed = (pos_contact_dirs_cam[:, :, 2:3] > z_val).repeat(
                1, 1, 3
            )
            pos_contact_points_cam = torch.where(
                dir_filter_passed,
                pos_contact_points_cam,
        ...
        pos_contact_points_cam_expanded = torch.unsqueeze(
            pos_contact_points_cam, 1
        )  # B x 1 x M x 3
        squared_dists_all = torch.sum(
            (xyz_cam_expanded - pos_contact_points_cam_expanded) ** 2, dim=3
        )  # B x N x M

        # B x N x k, B x N x k
        squared_dists_k, close_contact_pt_idcs = torch.topk(
            squared_dists_all, k=nsample, dim=2, largest=False, sorted=False
        )

        # -- Group labels -- #
        grouped_contact_dirs_cam = utils.index_points(
            pos_contact_dirs_cam, close_contact_pt_idcs
        )  # B x N x k x 3
        grouped_finger_diffs = utils.index_points(
            pos_finger_diffs, close_contact_pt_idcs
        )  # B x N x k x 1
        grouped_approach_dirs_cam = utils.index_points(
            pos_approach_dirs_cam, close_contact_pt_idcs
        )  # B x N x k x 3
        ...
        grasp_success_label = (
            torch.mean(squared_dists_k, dim=2, keepdim=True) < radius**2
        )  # B x N x 1
