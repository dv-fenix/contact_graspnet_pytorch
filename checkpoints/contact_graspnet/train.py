...
def train(global_config, log_dir):
    ...
    contact_infos = load_scene_contacts(
    ...
    with tf.Graph().as_default():
        ...
        saver = tf.train.Saver(
            save_relative_paths=True, keep_checkpoint_every_n_hours=4
        )
...
def train_one_epoch(sess, ops, summary_ops, file_writers, pcreader):
    ...
    for batch_idx in range(pcreader._num_train_samples):
        ...
        if (batch_idx + 1) % 10 == 0:
            ...
            f = tuple(np.mean(loss_log, axis=0)) + ((time.time() - get_time) / 10.0,)
            log_string(
                "total loss: %f \t dir loss: %f \t ce loss: %f \t off loss: %f \t app loss: %f adds loss: %f \t adds_gt2pred loss: %f \t batch time: %f"
                % f
            )
            get_time = time.time()
...
def eval_validation_scenes(
    sess, ops, summary_ops, file_writers, pcreader, max_eval_objects=500
):
    """ops: dict mapping from string to tf ops"""
    is_training = False
    log_string(str(datetime.now()))
    ...
    for batch_idx in np.arange(min(pcreader._num_test_samples, max_eval_objects)):
        ...
        (
            ...
            [
                ...
                ops["adds_loss"],
                ops["adds_gt2pred_loss"],
                summary_ops["merged_eval"],
                summary_ops["pr_update_op"],
                summary_ops["auc_update_op"],
            ]
            + [summary_ops["acc_update_ops"]],
            feed_dict=feed_dict,
        )
    ...
    log_string(
        "mean val loss: %f \t mean val dir loss: %f \t mean val ce loss: %f \t mean off loss: %f \t mean app loss: %f \t mean adds loss: %f \t mean adds_gt2pred loss: %f"
        % f
    )
