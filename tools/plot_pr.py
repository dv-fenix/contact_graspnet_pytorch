...
def metric_coverage_success_rate(
    ...
    for i in range(num_scenes):
        ...
        try:
            print(
                len(grasps_list[i]),
                len(scores_list[i].reshape(-1)),
                len(flex_outcomes_list[i]),
            )
        except:
            import pdb
            ...
            pdb.set_trace()
        for g, s, f in zip(
            grasps_list[i], scores_list[i].reshape(-1), flex_outcomes_list[i]
        ):
...
pr_data = (
    glob2.glob(os.path.join(sys.argv[1], "*", "*", "all_full_results.npz"))
    + glob2.glob(os.path.join(sys.argv[1], "*", "all_full_results.npz"))
    + glob2.glob(os.path.join(sys.argv[1], "all_full_results.npz"))
)
...
for abc in pr_data:
    try:
        x = np.load(
            os.path.join(os.path.dirname(abc), "flex_temp", "all_full_results.npz"),
            allow_pickle=True,
        )
    except:
        x = np.load(
            os.path.join(
                os.path.dirname(os.path.dirname(abc)),
                "flex_temp",
                "all_full_results.npz",
            ),
            allow_pickle=True,
        )
    ...
    outfile_data = (
        glob2.glob(os.path.join(base_dir, "*", "*", "outfile.npy"))
        + glob2.glob(os.path.join(base_dir, "*", "outfile.npy"))
        + glob2.glob(os.path.join(base_dir, "outfile.npy"))
    )
...
print(
    "{:<30}".format("Parameter") + stri.format("default", *[d for d in all_diff_dicts])
)
