                ...
                if os.path.exists(contact_path):
                    ...
                    if "contact_points" in npz:
                        ...
                        if len(pos_idcs) > min_pos_contacts:
                            ...
                            contact_infos[grasp_path]["grasp_transform"] = npz[
                                "grasp_transform"
                            ]
                            contact_infos[grasp_path]["contact_points"] = npz[
                                "contact_points"
                            ]
    if not contact_infos:
        print(
            "Warning: No mesh_contacts found. Please run tools/create_contact_infos.py first!"
        )
...
class TableScene(Scene):
    ...
    def _filter_colliding_grasps(self, transformed_grasps, transformed_contacts):
        ...
        return np.array(filtered_grasps).reshape(-1, 4, 4), np.array(
            filtered_contacts
        ).reshape(-1, 2, 3)
    ...
    def load_existing_scene(self, path):
        ...
        for obj_path, obj_transform, obj_scale in zip(
            obj_paths, obj_transforms, obj_scales
        ):
    ...
    def visualize(self, scene_grasps, scene_contacts=None):
        ...
        gripper_markers = [
            gripper_marker.copy().apply_transform(t) for t in scene_grasps
        ]

        colors = np.ones((scene_contacts.shape[0] * 2, 4)) * 255
        colors[:, 0:2] = 0
        scene_contact_scene = trimesh.Scene(
            trimesh.points.PointCloud(scene_contacts.reshape(-1, 3), colors=colors)
        )

        # show scene together with successful and collision-free grasps of all objects
        trimesh.scene.scene.append_scenes(
            [
                self.colorize().as_trimesh_scene(),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "root_folder",
        help="Root dir with grasps, meshes, mesh_contacts and splits",
        type=str,
    )
    ...
    parser.add_argument(
        "--gripper_path",
        type=str,
        default="gripper_models/panda_gripper/panda_gripper.obj",
    )
